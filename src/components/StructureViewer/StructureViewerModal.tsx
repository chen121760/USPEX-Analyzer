...
const SCRIPTS = {
  ...
  axes: 'axes unitcell; axes 0.1; axes on;',
  axesOff: 'axes off;',
...
const DEFAULT_DISPLAY_MODE = 'ballAndStick' as const;
...
export function StructureViewerModal() {
  ...
  const [displayMode, setDisplayMode] = useState<'ballAndStick' | 'spacefill' | 'wireframe' | 'sticks'>(DEFAULT_DISPLAY_MODE);
  const [supercell, setSupercell] = useState<[number, number, number]>(DEFAULT_SUPERCELL);
  ...
  const initScript = [
    ...
    showPolyhedra ? SCRIPTS.polyhedraOn : SCRIPTS.polyhedraOff,
    showAxes ? SCRIPTS.axes : SCRIPTS.axesOff,
    showLabels ? SCRIPTS.labels : SCRIPTS.labelsOff,
    spinning ? SCRIPTS.spin : SCRIPTS.spinOff,
  ].join(' ');
  ...
  const viewerLoadScript = isSupercell
